...
import pytest
import numpy as np
from gmi.linear_operator import FourierConvolution, FourierLinearOperator
...
class TestFourierConvolution:
    ...
    def test_inheritance(self):
        ...
        from gmi.linear_operator import (
            FourierLinearOperator,
            EigenDecompositionLinearOperator,
            LinearOperator
        )
        
        assert isinstance(conv, FourierLinearOperator)
        assert isinstance(conv, EigenDecompositionLinearOperator)
        assert isinstance(conv, LinearOperator)
    ...
    def test_mat_add(self):
        """Test matrix addition with other FourierLinearOperator instances."""
        ...
        assert isinstance(conv_sum, FourierLinearOperator)
        assert torch.allclose(conv_sum.filter, conv1.filter + conv2.filter)
    
    def test_mat_mul(self):
        """Test matrix multiplication with other FourierLinearOperator instances."""
        kernel1 = torch.randn(8, 8)
        kernel2 = torch.randn(8, 8)
        ...
        assert isinstance(conv_mul, FourierLinearOperator)
