"""
Tests for the InvertibleCompositeLinearOperator class.
"""
import pytest
import torch
from gmi.linear_operator.invertible_composite import InvertibleCompositeLinearOperator
...
class TestInvertibleCompositeLinearOperator:
    """Test cases for the InvertibleCompositeLinearOperator class."""
    
    def test_instantiation(self):
        """Test that InvertibleCompositeLinearOperator can be instantiated directly."""
        op1 = ScalarLinearOperator(scalar=2.0)
        op2 = ScalarLinearOperator(scalar=3.0)
        op = InvertibleCompositeLinearOperator(matrix_operators=[op1, op2])
    ...
    def test_inheritance(self):
        """Test that InvertibleCompositeLinearOperator inherits correctly."""
    ...
    def test_is_invertible_property(self):
        ...
        op_mixed = InvertibleCompositeLinearOperator(matrix_operators=[op1, op_zero])
        ...
        op_single = InvertibleCompositeLinearOperator(matrix_operators=[op1])
        assert op_single.is_invertible, "Single invertible operator should be invertible"
        
        # Test with single non-invertible operator
        op_single_zero = InvertibleCompositeLinearOperator(matrix_operators=[op_zero])
        ...
        op_all_zero = InvertibleCompositeLinearOperator(matrix_operators=[op_zero, op_zero])
        ...
        op_mixed_order = InvertibleCompositeLinearOperator(matrix_operators=[op_zero, op1, op2])
        assert not op_mixed_order.is_invertible, "Mixed operators with non-invertible should not be invertible"
    
    def test_config_instantiation(self):
        """Test that InvertibleCompositeLinearOperator can be instantiated from config."""
        cfg = compose(config_name="linear_operator/invertible_composite.yaml")
        op = instantiate(cfg.linear_operator)
        assert isinstance(op, InvertibleCompositeLinearOperator), "Should instantiate InvertibleCompositeLinearOperator from config"
    ...
    def test_empty_composite(self, sample_vector_2):
        ...
        # Note: This should raise an error since InvertibleCompositeLinearOperator requires at least one operator
        with pytest.raises(AssertionError, match="At least one operator should be provided"):
            op = InvertibleCompositeLinearOperator(matrix_operators=[])
    
    def test_single_operator(self, sample_vector_2):
        """Test invertible composite operator with single operator."""
        op1 = ScalarLinearOperator(scalar=2.0)
        op = InvertibleCompositeLinearOperator(matrix_operators=[op1])
    ...
    def test_multiple_operators(self, sample_vector_2):
        ...
        op = InvertibleCompositeLinearOperator(matrix_operators=[op1, op2, op3])
    ...
    def test_inverse_LinearOperator(self, sample_vector_2):
        ...
        assert isinstance(inv_op, InvertibleCompositeLinearOperator), "Should return InvertibleCompositeLinearOperator"
    ...
    def test_non_invertible_operator_handling(self, sample_vector_2):
        ...
        op = InvertibleCompositeLinearOperator(matrix_operators=[op1, op_zero])
        ...
        # inverse_LinearOperator should also raise an error
        with pytest.raises(ValueError, match="The scalar is zero"):
            op.inverse_LinearOperator() 