import torch
import pytest
from gmi.linear_operator import RowSparseLinearOperator

def make_identity_row_sparse(input_shape):
    # For a 2x2 image, flatten indices: 0 1 2 3
    # Each input pixel maps to its corresponding output pixel
    indices = torch.tensor([
        [[0, 1], [2, 3]]  # shape [1, 2, 2]
    ])
    weights = torch.ones_like(indices, dtype=torch.float32)
    return RowSparseLinearOperator(input_shape, input_shape, indices, weights)

@pytest.mark.parametrize("dtype", [torch.float32, torch.complex64])
def test_row_sparse_linear_operator(dtype):
    op = make_identity_row_sparse((2, 2))
